...
int main(int argc, char** argv)
{
   ...
   void* data = stb_file("assets/fonts/arial.ttf", NULL);
   ...
   auto start = Plutus::Time::micros();
   ypos = 60;
   memset(bitmap, 255, sizeof(bitmap));
   char text[128] = { 0 };
   stbsp_sprintf(text, "sdf bitmap height %d", (int)sdf_size);

   print_text(400, ypos + 30, text, 30 / sdf_size);
   ypos += 80;
   for (scale = 8.0; scale < 120.0; scale *= 1.33f) {
      stbsp_sprintf(text, scale == 8.0 ? small_sample : sample, (int)scale);
      print_text(80, ypos + scale, text, scale / sdf_size);
      ypos += scale * 1.05f + 20;
   }

   printf("elapse: %llu", Plutus::Time::micros() - start);
