...
namespace Plutus
{
	...
	void EditorUI::Init(float x, float y, Camera2D* cam)
	{
		...
		if (mCanvasHover = ImGui::IsItemHovered())
		{
			...
			if (mInput->onKeyDown("Ctrl"))
			{
				if (mInput->onKeyPressed("MouseLeft"))
					lastCoords = mInput->getMouseCoords();
				...
				{
					//get the mouseCoord;
					auto coords = mInput->getMouseCoords();
					...
					glm::vec2 newPos(camPos.x - result.x, camPos.y - result.y);
	...
	void EditorUI::EntityList()
	{
		...
		if (mInput->onKeyDown("Ctrl"))
		{
			...
			if (mInput->getMouseWheel() > 0 && scale < 6.0f)
			{
				mCamera->setScale(scale + 0.05f);
			}
			else if (mInput->getMouseWheel() < 0 && scale > 0.1f)
			{
				mCamera->setScale(scale - 0.05f);
			}
		}
			...
			if (ImGui::BeginMenu("Files"))
			{
				...
				if (ImGui::BeginMenu("Recent"))
				{
					...
					for (auto recent : mRecents)
					{
						...
						if (ImGui::Button(ICON_FA_TRASH_ALT "##rem-recent"))
						{
							toRemove = recent;
						}
						ImGui::PopStyleColor();
