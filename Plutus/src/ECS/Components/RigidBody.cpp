...
namespace Plutus
{
    RigidBody::RigidBody(Entity ent, BodyType type, const vec2f& offset, float isFixedRotation, bool isBullet) :
        mEnt(ent), mBodyType(type), mOffset(offset), mFixedRotation(isFixedRotation), mBullet(isBullet) {
    }
